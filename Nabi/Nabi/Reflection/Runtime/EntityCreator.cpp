...
namespace nabi::Reflection
{
	...
	namespace Creation
	{
		void OverrideEntityComponents(std::vector<ComponentData>& componentData,
			...
			PropertyData const* const propertyDataOverride)
		{
		...
		void ResolveEntityComponents(std::vector<ComponentData> const& entityComponents, entt::registry& registery, entt::entity const entity)
		{
		...
		void ResolveProperties(MetaECSTypeData const& data, entt::meta_any& metaObject)
		{
			...
			for (PropertyData const& propertyNode : data.m_Properties)
			{
				...
				if (I_XmlParserLog)
				{
					LOG(LOG_PREP, LOG_INFO, SPACE(INDENT_2) << "Found a property with id " << WRAP(propertyId, "'") << 
						" and value " << WRAP(propertyValue, "'") << ENDLINE);
				}
#endif // ifdef USE_DEBUG_UTILS
		...
		void AddEntityInfoComponentToEntity(entt::registry& registey, entt::entity const entity,
			entt::hashed_string const& entityGroupHash, std::string_view entityName)
		{
	...
	EntityCreator::EntityCreator(entt::registry& registery, EntityTemplateStore const&& entityTemplateStore)
		: m_Registery(registery)
		, m_EntityTemplateStore(entityTemplateStore)
	{
	}

	void EntityCreator::CreateEntity(EntityCreator::EntityCreationSettings const& entityCreationSettings)
	{
	...
	size_t EntityCreator::GetEntityStoreSize() const
	{
		return m_EntityTemplateStore.size();
	}

	void EntityCreator::ResolveEntityTemplateComponents(EntityTemplateData& entityTemplateData, EntityPropertyList const& entityOverriddenProperties)
	{
		/*
		  bloody hell
